...
const settingsFormSchema = z.object({
  ...
  logo: z.instanceof(File).optional().nullable(), // For new logo upload
  themePrimaryColor: z.string().regex(hslColorStringRegex, 'Must be a valid HSL color string (e.g., "210 40% 98%")'),
  themeAccentColor: z.string().regex(hslColorStringRegex, 'Must be a valid HSL color string'),
  themeBackgroundColor: z.string().regex(hslColorStringRegex, 'Must be a valid HSL color string'),
});
...
export default function AdminSettingsPage() {
  ...
  const { register, handleSubmit, control, setValue, reset, formState: { errors } } = useForm<SettingsFormValues>({
    ...
    async function loadSettings() {
      ...
      try {
        ...
        reset({
          ...
          logo: null, // Keep logo field null by default for new uploads
        ...
        setLogoPreview(settings.logoUrl); // Initialize preview with current logo
      } catch (error) {
        toast({
  ...
  useEffect(() => {
    ...
    if (selectedLogoFile) {
      ...
      setLogoPreview(currentLogoUrl); // Fallback to current logo if no new file
    }
    return () => {
  ...
  const onSubmit: SubmitHandler<SettingsFormValues> = (data) => {
    startSaveTransition(async () => {
      ...
      if (result.success && result.data) {
        ...
        reset({ // Reset form with new values, keeping logo field null
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <CustomDropzone
                ...
                maxSize={1 * 1024 * 1024} // 1MB limit for logo
                className="mt-1"
            />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Customize your site&apos;s color scheme. Enter HSL values (e.g., <code className="bg-muted text-muted-foreground px-1 py-0.5 rounded-sm text-xs">228 100% 98%</code>).
            Changes will be applied to <code className="bg-muted text-muted-foreground px-1 py-0.5 rounded-sm text-xs">globals.css</code>.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <Input id="themePrimaryColor" {...register('themePrimaryColor')} className="mt-1" placeholder="e.g., 50 90% 55%" />
            ...
            <div>
              ...
              <Input id="themeAccentColor" {...register('themeAccentColor')} className="mt-1" placeholder="e.g., 30 84% 51%" />
            ...
            <div>
              ...
              <Input id="themeBackgroundColor" {...register('themeBackgroundColor')} className="mt-1" placeholder="e.g., 228 100% 98%" />
              {errors.themeBackgroundColor && <p className="text-sm text-destructive mt-1">{errors.themeBackgroundColor.message}</p>}
            </div>
          </div>
           <div className="mt-2 p-3 border border-yellow-500/30 bg-yellow-500/10 rounded-md">
                <AlertTriangle className="h-5 w-5 text-yellow-600 inline-block mr-2" />
                <span className="text-xs text-yellow-700">
                    Note: Dark mode color variables in <code className="bg-yellow-200/50 px-0.5 rounded-sm">globals.css</code> will also be updated to these same HSL values. For distinct dark mode colors, manual CSS adjustments are currently needed.
                </span>
            </div>
        </CardContent>
      </Card>
