    ...
    async function fetchContent() {
      ...
      try {
        ...
        if (content) {
          ...
          // This case should ideally be handled by getSitePageContent returning a default or error object
          setError("Privacy policy content could not be loaded.");
        }
      } catch (err: any) {
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-3xl md:text-4xl">
            {isLoading ? <Loader2 className="h-8 w-8 animate-spin text-primary" /> : pageContent?.title || "Privacy Policy"}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <p>No content available for the privacy policy.</p>
          )}
